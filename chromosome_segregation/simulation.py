    ...
    while ds > ds_min:
        ...
        for i in range(sweep_length):
           ...
           # else:
            ...
            coords_n, w_n, k_n, probability, probability_native  = regrow_saw(n, 0, 0, 0, [], w=1, alpha=alpha, k=0, prob=1, prob_native=0, limits=limits)
            if len(coords_n) < n:
                failed_to_grow += 1
            # print('skipping, failed to grow')
            else:
        ...
        if rank == 0:
           ...
           for d in tmp_list_is_in_box:
             for k in d.keys():
               list_is_in_box_global[k].append(d[k])  


           for k in list_probs_global.keys():
             if len(list_probs_global[k][0] ) < 20000000:  
               list_probs_global[k] = [[item for sublist  in list_probs_global[k] for item in sublist ]]
           ...
           logging.info('fraction is in the box %s is: %s' %(limits, [list_is_in_box_global[k][0].count(True)/len(list_is_in_box_global[k][0]) for k in list_probs_global.keys() if len(list_is_in_box_global[k][0]) >0]))


           w_means = []; bins_ = 20; min_ = 0.0; mean_dev = 0.05
           for k in list_probs_global.keys():
             ...
             #w_mean = np.mean(bins[np.where((np.cumsum(vals1) >= 0.5 - mean_dev)  & (np.cumsum(vals1) <= 0.5 + mean_dev))])
             w_means.append(w_mean)
           #logging.info("weighted means params are: %i, %f"%(bins_, mean_dev))
